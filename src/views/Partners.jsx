            ...
            <div className='max-w-[1500px] mx-auto pb-32 pt-3 flex flex-col gap-y-10 relative z-10'>
                ...
                <motion.div className='flex flex-col md:flex-row gap-x-6 gap-y-6' variants={containerVariants} initial="hidden" ref={partnerRef} animate={`${partnerView ? "visible" : ""}`}>
                    {partners.map((partner, i) =>
                        <motion.div key={i} className="bg-blk p-8 rounded-[15px] flex flex-col gap-y-5 text-center items-center justify-between group hover:drop-shadow-hover transition-all ease-out duration-300 w-full h-auto lg:h-[23rem]" variants={childVariants}>
                            ...
                            <p className='pt-4 font-bold text-accent align-bottom'>Price: <a href="" className='font-normal text-wht hover:underline'>{partner.price}</a></p>
                        </motion.div>
                    )}
                </motion.div>
            </div>
